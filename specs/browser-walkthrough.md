...
| 2026-08-10 | ~~V (inside the open X-ray) cycles combination views~~ SUPERSEDED next day | first reading of the owner's V ask |
| 2026-08-11 | V cycles the three views: normal → construction x-ray (hologram) → load view; L JUMPS straight into the load view from anywhere, then cycles the load case (worst → weight → quake); G and the `?loads=1`/`?xray=1` params deleted with no mapping; ☰ menu gets View + Load buttons; the compact aim readout shows the ACTIVE stop's tile value, the I panel lists every combination for the tile with failure-mode labels and the governing row bold (HUD renders escaped rich text) | owner 2026-08-11 design session — contract in [fem-xray.md](fem-xray.md) |
