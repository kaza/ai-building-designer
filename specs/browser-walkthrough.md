...
| 2026-08-08 | Loads view (L key / ☰ menu): heat-colors the structure by the takedown's numbers — beams by true utilization, bearing walls by relative ULS line load; everything else ghosts to 10%; continuous FEM rainbow (blue→red) via per-element 1D gradient textures UV-mapped along the member — beams show their bending parabola (vertex colors could not: box beams have no midspan vertices), walls follow an 8-bucket sampled load profile; I shows q/M/util per element; data embedded at build from output/loads.json (experiment `load_takedown.py --emit-json`, rerun after model changes — absent file = toggle explains itself) | owner: "render me in 3D where I could see the loads on my concrete" — the RFEM-style view, KISS edition | Almir |
