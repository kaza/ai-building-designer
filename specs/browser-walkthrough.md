...
| 2026-08-10 | Ghost becomes the STRUCTURE VIEW: G cycles off → structure → structure+ghost. Structure: furniture/decor/ground HIDDEN (not ghosted), walls/slabs/roofs solid, load-bearing walls tinted brick-red (`__BEARING__` payload from building.json `load_bearing`) so the bearing lines read at a glance — the ArchiCAD "Core of Load-Bearing Elements Only" idea. Structure+ghost: non-bearing structure translucent, bearing walls seen through everything. URL flags: `struct` (state 1), `struct,ghost` (state 2); old `,ghost` links map to state 2. FEM handoff unchanged: one mode owns the materials | owner 2026-08-10: "I want to see the walls and structural things... why the hell should I care about furniture" | Almir + Claude |
