...
| Foundations | **E108 — geometric support-path check, FREE-standing columns only** (named exactly that, NOT foundation adequacy — Codex; scope narrowed 2026-08-15, owner): a free column's base must land on a footing, a wall top of the storey below, the on-grade slab, or an aligned column below (column-on-column continuity). A host-placed tie is supported by construction — it anchors into the ring beam and its host wall's load path (the wall's support is E104's business); demanding a pier under every tie manufactured ~20 fictional foundations per confined candidate and propped up the very consoles the architect's design is about. E050/E103/E104/E105 keep their walls-only scope; column loads enter no footing-pressure math in C1 |
